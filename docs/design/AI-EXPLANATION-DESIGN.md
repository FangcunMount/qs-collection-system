...
本节覆盖下文旧 Generation 接口及缓存恢复约定。小程序只使用 QS Collection 新工作流入口；旧 AI 历史结果和本地旧指针不迁移。

- 来源：`GET /assessments/{id}/ai-workflows/source?testee_id=...`，返回经过当前权限校验的不可变 `report_id` 和来源版本。`ready` 仅表示事实就绪；配置选择与预算准入由 qs-ai 决定。
- 发起：`POST /assessments/{id}/ai-workflows?testee_id=...`，提交固定的 `request_id` UUID 和 `report_id`。点击后先生成随机 UUID 并持久保存两个标识，再发送请求；无法保存时不提交。
- 查询：`GET /assessments/{id}/ai-workflows/{request_id}?testee_id=...`。accepted/queued/running 映射为等待，completed 验证完整成果后展示，blocked/cancelled 显示未完成。当前页面内部 `generation_id`/路由 `gid` 仅承载新请求 UUID，不接受旧 Generation ID。
- 网络结果不明、页面重进或 GET 404 均保留原请求。自动恢复只查询；用户明确重试时重放同一个请求 ID 和报告 ID，不创建第二次测评。另一个页面已保存请求时，当前页面改为查询该请求。
- 恢复元数据使用账号、受试者、测评隔离的新存储键；不存报告正文、模型输出或凭据，也不按旧 7 天/30 条规则自动淘汰未决请求。退出登录仍清除会话私有状态，当前权限拒绝立即撤下结果。
- 展示完成结果时重新核对当前来源，报告 ID 不同时显示旧版本提示；暂时无法核实时不冒充当前报告。账号变化、页面隐藏和迟到响应继续受生命周期保护。

本批为代码与自动化验证，微信真机、真实 IAM 权限及生产模型闭环分别验收。后台与小程序同批切换后才关闭旧 AI 总开关。
