...
| 报告正文 | `{base}/typology-assessments/{id}/report?testee_id={testee}` | `{base}/assessments/{id}/report?testee_id={testee}` |
...
report_ready           → status=interpreted 后读取人格或医学 report
...
- [ ] JWT 可访问 `report-status` 与报告正文（人格与医学 collection `/report`）
