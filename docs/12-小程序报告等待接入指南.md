...
> **R121 破坏性变更**：`GET /api/v1/answersheets/{id}/assessment` 与过渡期 `/api/v2/assessments*` 均已下线。`GET /answersheets/submit-status` 只有在 `status=done` 且同时带有 `answersheet_id`、`assessment_id` 时才表示提交完成；`done` 缺任一 ID 是提交契约异常。按 `answer_sheet_id` 的人格或行为能力列表匹配只用于恢复历史链接，不能作为新提交的兜底。
...
sequenceDiagram
    ...
    Note over MP: submit 后轮询 submit-status，done 后原子取得 answersheet_id 与 assessment_id
...
```text
submit_pending         → 按 request_id 轮询 submit-status
assessment_ready       → submit-status=done 且持有 answersheet_id、assessment_id
