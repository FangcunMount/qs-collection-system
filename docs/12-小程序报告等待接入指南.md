...
> **R121 破坏性变更**：`GET /api/v1/answersheets/{id}/assessment` 与过渡期 `/api/v2/assessments*` 均已下线。`GET /answersheets/submit-status` 只有在 `status=done` 且同时带有 `answersheet_id`、`assessment_id` 时才表示提交完成；`done` 缺任一 ID 是提交契约异常。人格按 `answer_sheet_id` 的列表匹配只用于恢复历史链接，不能作为新提交的兜底。
...
| 报告正文 | `{base}/typology-assessments/{id}/report?testee_id={testee}` | `{base}/assessments/{id}/scores?testee_id={testee}`，前端映射为医学报告 ViewModel |
...
```http
GET /api/v1/assessments/{id}/scores?testee_id={testee_id}
Authorization: Bearer <token>
```

> collection 不提供 `GET /api/v1/assessments/{id}/report`；医学线在 `report-status=interpreted` 后读取 `scores`，由前端映射为报告 ViewModel。
...
sequenceDiagram
    ...
    alt 有 assessment_id
        ...
        alt 人格
            ...
            MP->>CS: GET /assessments/{id}/scores
        end
        CS-->>MP: 报告 JSON
    end
```
...
report_ready           → status=interpreted 后读取人格 report 或医学 scores
...
- [ ] JWT 可访问 `report-status` 与报告正文（人格 collection `/report`；医学 collection `/scores`）
...
- OpenAPI：`api/rest/collection.yaml`（`/report-events`、`/report-status`、`/wait-report`、人格 `/report` 与医学 `/scores`）
