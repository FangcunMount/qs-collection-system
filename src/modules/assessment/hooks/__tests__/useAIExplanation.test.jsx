...
jest.mock('@/services/api/aiExplanationApi', () => ({ ...jest.requireActual('@/services/api/aiExplanationApi'), getAIExplanationCapability: jest.fn(), requestAIExplanation: jest.fn(), getAIExplanation: jest.fn() }));
jest.mock('@/services/api/account', () => ({ getAccountProfile: jest.fn() }));
jest.mock('../../services/aiExplanationRequestId', () => ({createAIRequestId: jest.fn().mockResolvedValue('00000000-0000-4000-8000-000000000001')}));
let hook, tree, logs;
...
test('route testee change cancels old request and cannot render old content', async () => {
  ...
  const lifetime = api.requestAIExplanation.mock.calls[0][2];
...
test('account invalidation still clears content when storage removal fails',async()=>{
 await settle(()=>{tree=renderer.create(<Harness/>)});await settle(()=>{void hook.start()});
